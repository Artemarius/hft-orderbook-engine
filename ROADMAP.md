...
**Goal:** Ingest real Binance historical data, replay through the engine, reconstruct book state.

### 6.1 Data Format

Binance provides historical order-by-order data in CSV format. Download from: https://data.binance.vision/

Relevant fields per message: `timestamp, event_type (add/cancel/trade), order_id, side, price, quantity`

### 6.2 Feed Parser (`src/feed/l3_replay.h`)

- Parse CSV line by line (cold path — `std::ifstream` is fine)
- Convert each line to an `OrderMessage` (POD)
- Push into the gateway (either directly or via ring buffer)
- Support playback speed control: real-time, fast-forward, max-speed

### 6.3 Book Reconstruction Validation

After replaying a sequence:
- Best bid/ask should match known exchange state at that timestamp
- Trade messages in the feed should match trades generated by the engine
- This validates that matching logic is correct against real exchange behavior

### 6.4 Phase 6 Deliverables Checklist

- [ ] CSV parser for Binance L3 format
- [ ] Full replay through matching engine
- [ ] Book state validated against known snapshots
- [ ] `replay` binary works: `./build/replay --input data/btcusdt_l3.csv`
- [ ] Small sample data (5K messages) checked into `data/`
